...
#      }
#   
def create_rooted_spanning_tree0(G, node, S, visited_nodes, visited_edges):
    for node in G:
        if not node in visited_nodes:
            visited_nodes[node] =  {}
            for connected_node in G[node]:
                
                if node(node,connected_node) in visited_edges:
                    S[node][connected_node]=  'red'
...
def create_rooted_spanning_tree(G, root):
    ...
    visited_nodes = {}
    visited_edges = {}
    create_rooted_spanning_tree0(G,  root, S,  visited_nodes, visited_edges)
